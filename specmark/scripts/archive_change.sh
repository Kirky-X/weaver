...
# Windows 常见 python3 为 Microsoft Store 桩（执行即失败），优先用真实 python
...
# 独占锁，最多等 10 秒。有 flock（Linux util-linux）用 fd 9；
# Windows Git Bash 等无 flock 环境回退 mkdir 原子锁（跨平台，规则 31）。
...
if [[ $SYNC -eq 1 ]]; then
  ...
  if [[ ${#DELTA_SPECS[@]} -gt 0 ]]; then
    for delta in "${DELTA_SPECS[@]}"; do
      ...
      "$PYTHON" "$SCRIPT_DIR/merge_delta_spec.py" \
