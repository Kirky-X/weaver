...
Weaver 支持通过 PgBouncer 代理 PostgreSQL 连接，优化生产环境连接管理。
...
1. 安装 PgBouncer: `apt install pgbouncer`（Ubuntu）或等效方式
2. 配置 PgBouncer: 设置 `auth_type`, 数据库连接字符串
3. 配置 Weaver: 设置 `WEAVER_PGBOUNCER__ENABLED=true`
4. Weaver 自动将 DSN 指向 PgBouncer 代理地址

**池化模式**:

- `transaction`: 事务结束后释放连接（推荐，适合大多数场景）
- `session`: 客户端断开后释放连接（适合需要会话级状态的场景）
- `statement`: 每条语句后释放连接（最激进，不适合事务）
