...
The system SHALL support building queries to discover all relation types for an entity.

#### Scenario: Build relation types query
- **WHEN** calling `build_relation_types_query(entity_name, entity_type)`
- **THEN** it returns Cypher to query all relation types for the entity
- **AND** Neo4j version uses `type(r)`
- **AND** LadybugDB version uses `r.edge_type`

#### Scenario: Exclude system relations
- **WHEN** building relation types query
- **THEN** it excludes MENTIONS and FOLLOWED_BY relations

### Requirement: Related entities with relation filter query

The system SHALL support building queries for related entities filtered by relation type.

#### Scenario: Build filtered related entities query
- **WHEN** calling `build_related_entities_by_relation_query(entity_name, relation_types)`
- **THEN** it returns query filtered by specific relation types
- **AND** it supports filtering by multiple relation types simultaneously

### Requirement: Entity repository query support

The system SHALL support all query patterns required by EntityRepository.

#### Scenario: Merge entity query
- **WHEN** building entity MERGE query
- **THEN** it uses database-compatible syntax

#### Scenario: Find entity query
- **WHEN** building entity lookup query
- **THEN** it supports lookup by name and type
