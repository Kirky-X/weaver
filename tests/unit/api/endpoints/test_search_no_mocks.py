...
:mod:`api.dependencies` provides the required service getters.
"""
...
class TestSearchEndpointNoMocks:
    ...
    def test_no_mock_embedding_service(self, search_source: str) -> None:
        ...
        assert "MockEmbeddingService" not in search_source, (
            "search.py contains 'MockEmbeddingService' — use "
            "api.dependencies.get_embedding_service() instead"
        )
    ...
    def test_no_mock_intent_classifier(self, search_source: str) -> None:
        ...
        assert "MockIntentClassifier" not in search_source, (
            "search.py contains 'MockIntentClassifier' — use "
            "api.dependencies.get_intent_classifier() instead"
        )
    ...
    def test_no_inline_mock_class(self, search_source: str) -> None:
        ...
        assert "class Mock" not in search_source, (
            "search.py contains inline 'class Mock' — inject real services "
            "via api.dependencies getters"
        )
...
class TestEmbeddingServiceDependencyInjection:
    """Verify that api.dependencies exposes an embedding service getter."""

    def test_dependencies_has_get_embedding_service(self) -> None:
        """api.dependencies module must have a get_embedding_service function."""
        import api.dependencies

        assert hasattr(api.dependencies, "get_embedding_service"), (
            "api.dependencies is missing 'get_embedding_service' function — "
            "add it so search endpoints can obtain a real EmbeddingService"
        )

    def test_get_embedding_service_is_callable(self) -> None:
        """get_embedding_service must be a callable function, not a plain attribute."""
        from api.dependencies import get_embedding_service

        assert callable(get_embedding_service), (
            "'get_embedding_service' in api.dependencies must be a callable function"
        )
...
class TestIntentClassifierDependencyInjection:
    """Verify that api.dependencies exposes an intent classifier getter."""

    def test_dependencies_has_get_intent_classifier(self) -> None:
        """api.dependencies module must have a get_intent_classifier function."""
        import api.dependencies

        assert hasattr(api.dependencies, "get_intent_classifier"), (
            "api.dependencies is missing 'get_intent_classifier' function — "
            "add it so search endpoints can obtain a real IntentClassifier"
        )

    def test_get_intent_classifier_is_callable(self) -> None:
        """get_intent_classifier must be a callable function, not a plain attribute."""
        from api.dependencies import get_intent_classifier

        assert callable(get_intent_classifier), (
            "'get_intent_classifier' in api.dependencies must be a callable function"
        )
