...
class TestVectorizeNodeBasic:
    ...
    async def test_vectorize_successful(self, mock_llm, sample_raw):
        ...
        mock_llm.batch_embed = AsyncMock(return_value=[mock_embedding])
    ...
    @pytest.mark.asyncio
    async def test_vectorize_calls_batch_embed_correctly(self, mock_llm, sample_raw):
        """Test that vectorize calls batch_embed with correct text."""
        ...
        mock_llm.batch_embed.assert_called_once()
        call_args = mock_llm.batch_embed.call_args

        # Verify the text format: title + newline + body (truncated to 2000)
...
class TestVectorizeNodeErrorHandling:
    ...
    async def test_vectorize_handles_embedding_error(self, mock_llm, sample_raw):
        ...
        mock_llm.batch_embed = AsyncMock(side_effect=Exception("Embedding service unavailable"))
    ...
    async def test_vectorize_handles_timeout(self, mock_llm, sample_raw):
        ...
        mock_llm.batch_embed = AsyncMock(side_effect=TimeoutError("Request timeout"))
    ...
    async def test_vectorize_handles_empty_embedding_result(self, mock_llm, sample_raw):
        ...
        mock_llm.batch_embed = AsyncMock(return_value=[])
...
class TestVectorizeNodeIntegration:
    ...
    async def test_vectorize_preserves_state(self, mock_llm, sample_raw):
        ...
        mock_llm.batch_embed = AsyncMock(return_value=[[0.2] * 1536])
    ...
    async def test_vectorize_with_different_embedding_dimensions(self, mock_llm, sample_raw):
        ...
        mock_llm.batch_embed = AsyncMock(return_value=[[0.1] * 512])
    ...
    async def test_vectorize_multiple_calls_consistency(self, mock_llm, sample_raw):
        ...
        embedding1 = [0.1] * 1536
        embedding2 = [0.2] * 1536
        mock_llm.batch_embed = AsyncMock(side_effect=[[embedding1], [embedding2]])
