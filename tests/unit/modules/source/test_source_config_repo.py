...
"""Unit tests for SourceConfigRepo."""
...
class TestSourceConfigRepoGetCredibility:
    ...
    @pytest.mark.asyncio
    async def test_get_credibility_fallback_matches_parsed_host(self):
        """Fallback hit: SourceConfig row whose URL authority equals host."""
        mock_pool = MagicMock()
        mock_session = AsyncMock()
        mock_pool.session.return_value.__aenter__.return_value = mock_session
    ...
    async def test_get_credibility_fallback_rejects_host_in_path(self):
        ...
        mock_source = MagicMock()
        mock_source.url = "https://a.com/example.com/feed"
...
class TestSourceConfigRepoUpsert:
    ...
    async def test_upsert_update_existing(self):
        ...
        mock_source = MagicMock()
        mock_source.id = "existing-id"
        mock_source.name = "Updated Source"
        mock_source.url = "https://updated.com/feed.xml"
        mock_source.source_type = "rss"
        mock_source.enabled = False
        mock_source.interval_minutes = 30
        mock_source.per_host_concurrency = 1
        mock_source.credibility = 0.7
        mock_source.tier = 2
        mock_source.last_crawl_time = None
        mock_source.etag = None
...
class TestSourceConfigRepoUpdateCrawlState:
    ...
    @pytest.mark.asyncio
    async def test_update_crawl_state_full(self):
        """Test update_crawl_state with all parameters."""
        mock_pool = MagicMock()
        mock_session = AsyncMock()
        mock_pool.session.return_value.__aenter__.return_value = mock_session
