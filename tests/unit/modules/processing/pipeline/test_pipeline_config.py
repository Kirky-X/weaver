...
"""Unit tests for processing pipeline configuration."""
...
class TestBatchConfig:
    ...
    def test_custom_values(self):
        batch = BatchConfig(merger_class="custom.Merger", enabled=False, timeout=300)
...
class TestPipelineConfig:
    ...
    def test_to_dict_stage_fields(self):
        config = PipelineConfig.default()
        data = config.to_dict()
        for stage in data["pipeline"]["phase1"]["stages"]:
...
class TestDictToStage:
    ...
    def test_extra_params(self):
        stage = _dict_to_stage(
            {"name": "extra", "class": "extra.Module", "custom_param": "value", "another": 123}
        )
...
class TestDictToPhase:
    ...
    def test_with_stages(self):
        phase = _dict_to_phase(
            {
                "concurrency": 10,
                "stages": [{"name": "s1", "class": "test.S1"}, {"name": "s2", "class": "test.S2"}],
            }
        )
...
class TestDictToBatch:
    ...
    def test_custom_values(self):
        batch = _dict_to_batch({"merger_class": "custom.Merger", "enabled": False, "timeout": 200})
        assert batch.merger_class == "custom.Merger"
        assert batch.enabled is False
...
class TestDictToConfig:
    ...
    def test_full_config(self):
        data = {
            "pipeline": {
                "version": "2.0",
                "phase1": {"concurrency": 3, "stages": [{"name": "s1", "class": "test.S1"}]},
                "phase3": {"concurrency": 2, "stages": []},
            }
        }
...
class TestPipelineConfigLoader:
    ...
    def test_load_with_env_override_concurrency(self):
        ...
        with patch.dict(
            os.environ,
            {"WEAVER_PHASE1_CONCURRENCY": "10", "WEAVER_PHASE3_CONCURRENCY": "20"},
        ):
            config = loader.load_with_env_override()
    ...
    def test_load_with_env_override_from_file(self):
        loader = PipelineConfigLoader()
        with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
            yaml.dump(
                {
            ...
            f.flush()
            with patch.dict(os.environ, {"WEAVER_PIPELINE_CONFIG": f.name}, clear=False):
                config = loader.load_with_env_override()
