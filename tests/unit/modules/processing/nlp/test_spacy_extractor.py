...
class TestModelPathConfiguration:
    """Tests for model path configuration via constructor parameters."""

    def test_init_with_zh_model_path(self) -> None:
        """Test that zh_model_path is stored correctly."""
        extractor = SpacyExtractor(zh_model_path="/path/to/zh_model.whl")
        assert extractor._zh_model_path == "/path/to/zh_model.whl"
        assert extractor._en_model_path is None

    def test_init_with_en_model_path(self) -> None:
        """Test that en_model_path is stored correctly."""
        extractor = SpacyExtractor(en_model_path="/path/to/en_model.whl")
        assert extractor._zh_model_path is None
        assert extractor._en_model_path == "/path/to/en_model.whl"

    def test_init_with_both_model_paths(self) -> None:
        """Test that both model paths are stored correctly."""
        extractor = SpacyExtractor(
            zh_model_path="/path/to/zh_model.whl",
            en_model_path="/path/to/en_model.whl",
        )
        assert extractor._zh_model_path == "/path/to/zh_model.whl"
        assert extractor._en_model_path == "/path/to/en_model.whl"

    def test_init_with_default_paths(self) -> None:
        """Test that default model paths are None."""
        extractor = SpacyExtractor()
        assert extractor._zh_model_path is None
        assert extractor._en_model_path is None
    ...
    def test_load_falls_back_to_installed_model_when_no_config(
        ...
        extractor = SpacyExtractor()  # No config paths

        # Call _load - should try spacy.load with model name
        result = extractor._load("zh_core_web_lg")
