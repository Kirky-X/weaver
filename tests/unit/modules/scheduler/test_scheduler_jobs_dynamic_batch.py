...
@pytest.fixture
def scheduler_jobs_service_with_settings():
...
@pytest.fixture
def scheduler_jobs_service_no_dynamic():
...
class TestGetRecentSuccessRate:
    ...
    @pytest.mark.asyncio
    async def test_get_recent_success_rate_returns_1_0_when_no_data(self, scheduler_jobs_service):
        """Test returns 1.0 when no data exists in Redis."""
        scheduler_jobs_service._cache.get = AsyncMock(return_value=None)

        rate = await scheduler_jobs_service._get_recent_success_rate()

        assert rate == 1.0

    @pytest.mark.asyncio
    async def test_get_recent_success_rate_returns_stored_value(self, scheduler_jobs_service):
        """Test returns stored success rate from Redis."""
        scheduler_jobs_service._cache.get = AsyncMock(return_value="0.85")
    ...
    @pytest.mark.asyncio
    async def test_get_recent_success_rate_handles_invalid_value(self, scheduler_jobs_service):
        """Test returns 1.0 for invalid values."""
        scheduler_jobs_service._cache.get = AsyncMock(return_value="invalid")
    ...
    @pytest.mark.asyncio
    async def test_get_recent_success_rate_handles_type_error(self, scheduler_jobs_service):
        """Test returns 1.0 for type errors."""
        scheduler_jobs_service._cache.get = AsyncMock(side_effect=TypeError("Unexpected type"))
...
class TestRetryPipelineProcessingDynamicBatch:
    ...
    async def test_dynamic_batch_disabled_uses_default_size(
        self, scheduler_jobs_service_no_dynamic
    ):
        """Test uses default batch size when dynamic batching disabled."""
        scheduler_jobs_service_no_dynamic._article_repo.get_pending = AsyncMock(
            return_value=[MagicMock()]
        )
        scheduler_jobs_service_no_dynamic._article_repo.get_stuck_articles = AsyncMock(
            return_value=[]
        )
        scheduler_jobs_service_no_dynamic._article_repo.get_failed_articles = AsyncMock(
            return_value=[]
        )
        scheduler_jobs_service_no_dynamic._pipeline.process_batch = AsyncMock()

        await scheduler_jobs_service_no_dynamic.retry_pipeline_processing()

        # Verify default batch size was used
        scheduler_jobs_service_no_dynamic._article_repo.get_pending.assert_called_once_with(
            limit=20
        )

    @pytest.mark.asyncio
    async def test_dynamic_batch_doubles_when_high_success_rate(
        self, scheduler_jobs_service_with_settings
    ):
        """Test doubles batch size when success rate >= threshold."""
        # Mock success rate of 0.9 (above 0.8 threshold)
        scheduler_jobs_service_with_settings._cache.get = AsyncMock(return_value="0.9")
        scheduler_jobs_service_with_settings._article_repo.get_pending = AsyncMock(
            return_value=[MagicMock()]
        )
        scheduler_jobs_service_with_settings._article_repo.get_stuck_articles = AsyncMock(
            return_value=[]
        )
        scheduler_jobs_service_with_settings._article_repo.get_failed_articles = AsyncMock(
            return_value=[]
        )
        scheduler_jobs_service_with_settings._pipeline.process_batch = AsyncMock()

        await scheduler_jobs_service_with_settings.retry_pipeline_processing()

        # Verify doubled batch size (20 * 2 = 40)
        scheduler_jobs_service_with_settings._article_repo.get_pending.assert_called_once_with(
            limit=40
        )
    ...
    async def test_dynamic_batch_halves_when_low_success_rate(
        ...
        scheduler_jobs_service_with_settings._cache.get = AsyncMock(return_value="0.5")
        scheduler_jobs_service_with_settings._article_repo.get_pending = AsyncMock(
            return_value=[MagicMock()]
        )
        scheduler_jobs_service_with_settings._article_repo.get_stuck_articles = AsyncMock(
    ...
    async def test_dynamic_batch_caps_at_max_when_success_high(
        ...
        scheduler_jobs_service_with_settings._cache.get = AsyncMock(return_value="1.0")
        scheduler_jobs_service_with_settings._article_repo.get_pending = AsyncMock(
            return_value=[MagicMock()]
        )
        scheduler_jobs_service_with_settings._article_repo.get_stuck_articles = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_dynamic_batch_caps_at_max_50(self, scheduler_jobs_service_with_settings):
        """Test caps batch size at 50 when doubling would exceed."""
        from config.settings import SchedulerSettings

        jobs = scheduler_jobs_service_with_settings
    ...
    async def test_dynamic_batch_updates_success_rate_in_redis(
        ...
        scheduler_jobs_service_with_settings._cache.get = AsyncMock(return_value=None)
        scheduler_jobs_service_with_settings._cache.set = AsyncMock()
        scheduler_jobs_service_with_settings._article_repo.get_pending = AsyncMock(
        ...
        scheduler_jobs_service_with_settings._cache.set.assert_called_once_with(
    ...
    async def test_dynamic_batch_calculates_correct_success_rate(
        ...
        scheduler_jobs_service_with_settings._article_repo.mark_failed = AsyncMock()

        await scheduler_jobs_service_with_settings.retry_pipeline_processing()

        # Verify success rate was calculated correctly (2/3 = 0.666...)
        call_args = scheduler_jobs_service_with_settings._cache.set.call_args
    ...
    async def test_dynamic_batch_no_update_when_no_articles(
        ...
        scheduler_jobs_service_with_settings._cache.get = AsyncMock(return_value=None)
        scheduler_jobs_service_with_settings._cache.set = AsyncMock()
        scheduler_jobs_service_with_settings._article_repo.get_pending = AsyncMock(return_value=[])
        scheduler_jobs_service_with_settings._article_repo.get_stuck_articles = AsyncMock(
            return_value=[]
        )
        scheduler_jobs_service_with_settings._article_repo.get_failed_articles = AsyncMock(
        ...
        scheduler_jobs_service_with_settings._cache.set.assert_not_called()

    @pytest.mark.asyncio
    async def test_dynamic_batch_no_update_when_disabled(self, scheduler_jobs_service_no_dynamic):
        """Test does not update success rate when dynamic batching disabled."""
        scheduler_jobs_service_no_dynamic._cache.get = AsyncMock(return_value=None)
        scheduler_jobs_service_no_dynamic._cache.set = AsyncMock()
        scheduler_jobs_service_no_dynamic._article_repo.get_pending = AsyncMock(
            return_value=[MagicMock()]
        )
        scheduler_jobs_service_no_dynamic._article_repo.get_stuck_articles = AsyncMock(
        ...
        # Verify success rate was NOT updated
        scheduler_jobs_service_no_dynamic._cache.set.assert_not_called()
    ...
    @pytest.mark.asyncio
    async def test_dynamic_batch_with_exact_threshold(self, scheduler_jobs_service_with_settings):
        """Test uses doubled batch size when success rate equals threshold."""
        # Mock success rate exactly at threshold (0.8)
        scheduler_jobs_service_with_settings._cache.get = AsyncMock(return_value="0.8")
        scheduler_jobs_service_with_settings._article_repo.get_pending = AsyncMock(
            return_value=[MagicMock()]
        )
        scheduler_jobs_service_with_settings._article_repo.get_stuck_articles = AsyncMock(
