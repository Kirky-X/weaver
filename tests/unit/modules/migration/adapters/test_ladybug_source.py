...
class TestLadybugSourceSecurity:
    ...
    @pytest.fixture
    def ladybug_source(self, graph_mock_pool):
        """Create a LadybugSource instance with mock pool."""
        return LadybugSource(graph_mock_pool)
    ...
    @pytest.mark.asyncio
    async def test_read_nodes_accepts_valid_labels(self, ladybug_source, graph_mock_pool, label):
    ...
    @pytest.mark.asyncio
    async def test_count_nodes_uses_parameterized_query(self, ladybug_source, graph_mock_pool):
        """count_nodes should use parameterized query, not f-string."""
        graph_mock_pool.execute_query.return_value = [{"count": 42}]
    ...
    @pytest.mark.asyncio
    async def test_read_rels_uses_parameterized_query(self, ladybug_source, graph_mock_pool):
    ...
    @pytest.mark.asyncio
    async def test_prevents_sql_injection_in_label(self, ladybug_source, graph_mock_pool):
    ...
    @pytest.mark.asyncio
    async def test_prevents_sql_injection_in_rel_type(self, ladybug_source, graph_mock_pool):
...
class TestLadybugSourceFunctionality:
    ...
    @pytest.mark.asyncio
    async def test_count_nodes_returns_correct_count(self, ladybug_source, graph_mock_pool):
        """count_nodes should return the count from query result."""
        graph_mock_pool.execute_query.return_value = [{"count": 123}]
    ...
    @pytest.mark.asyncio
    async def test_count_nodes_returns_zero_on_empty_result(self, ladybug_source, graph_mock_pool):
    ...
    @pytest.mark.asyncio
    async def test_get_label_names_returns_distinct_labels(self, ladybug_source, graph_mock_pool):
