...
class TestNewsNowParserParseDate:
    ...
    def test_parse_date_very_large_timestamp(self):
        """Test handling very large timestamp that causes overflow."""
        # This timestamp is so large that even after dividing by 1000,
        # it's still far beyond the valid range for datetime
        entry = {"extra": {"date": 9999999999999999}}

        result = NewsNowParser._parse_date(entry)

        # Should return None due to OverflowError/OSError
        assert result is None

    def test_parse_date_negative_timestamp(self):
        """Test handling negative timestamp."""
        entry = {"extra": {"date": -1}}

        result = NewsNowParser._parse_date(entry)

        # Negative timestamp returns a valid datetime (1969-12-31 23:59:59 UTC)
        assert result is not None
