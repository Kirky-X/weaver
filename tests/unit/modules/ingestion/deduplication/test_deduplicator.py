...
class TestDeduplicatorInit:
    ...
    def test_deduplicator_with_custom_ttl(self):
        ...
        dedup = Deduplicator(redis=mock_redis, article_repo=mock_repo, ttl_seconds=3600)
...
class TestDeduplicatorDedup:
    ...
    @pytest.mark.asyncio
    async def test_dedup_all_new_items(self, mock_redis, mock_repo, mock_items):
    ...
    @pytest.mark.asyncio
    async def test_dedup_filters_redis_duplicates(self, mock_redis, mock_repo):
        """Test Redis first-level deduplication filters existing URLs."""
        # First URL exists in Redis, second is new
        pipeline_mock = MagicMock()
        pipeline_mock.execute = AsyncMock(return_value=[True, False])
        pipeline_mock.hset = MagicMock()
    ...
    @pytest.mark.asyncio
    async def test_dedup_filters_db_duplicates(self, mock_redis, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_dedup_all_filtered(self, mock_redis, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_dedup_without_repo_get_existing_urls(self, mock_redis):
        """Test dedup when repo doesn't have get_existing_urls method."""
        pipeline_mock = MagicMock()
        pipeline_mock.execute = AsyncMock(return_value=[False])
        pipeline_mock.hset = MagicMock()
        mock_redis.pipeline = MagicMock(return_value=pipeline_mock)
...
class TestDeduplicatorDedupUrls:
    ...
    @pytest.mark.asyncio
    async def test_dedup_urls_empty_list(self, mock_redis, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_dedup_urls_all_new(self, mock_redis, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_dedup_urls_filters_redis_duplicates(self, mock_redis, mock_repo):
...
class TestDeduplicatorCleanupExpired:
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_no_entries(self, mock_redis, mock_repo):
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_with_old_entries(self, mock_redis, mock_repo):
        ...
        dedup = Deduplicator(redis=mock_redis, article_repo=mock_repo, ttl_seconds=86400 * 7)
    ...
    @pytest.mark.asyncio
    async def test_cleanup_expired_with_invalid_timestamp(self, mock_redis, mock_repo):
