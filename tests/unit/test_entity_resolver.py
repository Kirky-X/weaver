...
class TestEntityResolver:
    ...
    async def test_exact_match(self, resolver, mock_entity_repo):
        ...
        mock_entity_repo.find_entity = AsyncMock(return_value={
            "neo4j_id": "existing_id",
            "canonical_name": "张三"
        })
    ...
    async def test_vector_similarity_candidates(self, resolver, mock_vector_repo, mock_entity_repo):
        ...
        mock_vector_repo.find_similar_entities = AsyncMock(return_value=[
            MagicMock(neo4j_id="candidate_1", similarity=0.9)
        ])
        mock_entity_repo.find_entity_by_id = AsyncMock(return_value={
            "neo4j_id": "candidate_1",
            "canonical_name": "张三",
            "similarity": 0.9
        })
    ...
    async def test_merge_with_existing(self, resolver, mock_llm, mock_entity_repo, mock_vector_repo):
        ...
        mock_entity_repo.find_entity = AsyncMock(return_value=None)
        mock_vector_repo.find_similar_entities = AsyncMock(return_value=[
            MagicMock(neo4j_id="existing_id", similarity=0.95)
        ])
        mock_entity_repo.find_entity_by_id = AsyncMock(return_value={
            "neo4j_id": "existing_id",
            "canonical_name": "张三",
    ...
    async def test_llm_deduplicate_merge(self, resolver, mock_llm, mock_entity_repo, mock_vector_repo):
        ...
        mock_entity_repo.find_entity = AsyncMock(return_value=None)
        mock_vector_repo.find_similar_entities = AsyncMock(return_value=[
            MagicMock(neo4j_id="existing_id", similarity=0.88)
        ])
        mock_entity_repo.find_entity_by_id = AsyncMock(return_value={
            "neo4j_id": "existing_id",
            "canonical_name": "张三",
    ...
    @pytest.mark.asyncio
    async def test_llm_deduplicate_no_merge(self, resolver, mock_llm, mock_entity_repo, mock_vector_repo):
        """Test LLM deduplication decides not to merge."""
        mock_entity_repo.find_entity = AsyncMock(return_value=None)
        mock_vector_repo.find_similar_entities = AsyncMock(return_value=[
            MagicMock(neo4j_id="existing_id", similarity=0.86)
        ])
        mock_entity_repo.find_entity_by_id = AsyncMock(return_value={
            "neo4j_id": "existing_id",
            "canonical_name": "李四",
