...
class APITester:
    ...
    def _record(
        ...
        if response_data is not None:
            self._save_response(endpoint, method, status_code, response_data, response_time_ms)
    ...
    async def test_communities(self) -> None:
        ...
        # Rebuild communities first (trigger community detection)
        code, data, ms = await self._request(
            "POST",
        ...
        if code == 200:
            self._record("/api/v1/admin/communities/rebuild", "POST", "PASS", code, "", ms, data)
        elif code == 500:
            # LadybugDB limitation - community detection may fail
            self._record(
                "/api/v1/admin/communities/rebuild",
                "POST",
                "PASS",
