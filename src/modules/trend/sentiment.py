...
"""Sentiment trend analyzer — implements SentimentTrendProtocol.



SentimentTrendAnalyzer queries the ``sentiment_shifts`` table for article-level

shifts (SentimentTrackerNode output, article_id IS NOT NULL) within a
...
- ``entity_name`` path: WHERE entity_name = ? AND article_id IS NOT NULL.
  ...
  Used by endpoint ``GET /api/v1/trends/sentiment?entity=xxx``.
...
- ``community_id`` path: WHERE community_id = ?. Note that for article-level
  ...
  we only aggregate signed shift values from (community-level rows
...
Failure handling (Rule 12 — fail loud):
    ...
    Validation errors (both None / invalid window_days)
...
class SentimentTrendAnalyzer:
    ...
    async def analyze_trend(
        ...
        # Compute avg_shift = mean(shift_value), reusing the per-record values

        # already normalized in `shifts` (avoids a second Decimal->float pass

        # and guarantees both views agree).

        shift_values = [s["shift_value"] for s in shifts if s["shift_value"] is not None]
