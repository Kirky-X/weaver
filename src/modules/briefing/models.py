...
class BriefingResult:
    """Result returned by DailyBriefingProtocol methods.

    Used by:
        - DailyBriefingService.generate_briefing / get_briefing / list_briefings
        - briefings API endpoint (serialized to APIResponse[BriefingResult])
    ...
    Fields:
        ...
        False for template-mode briefings or when narrative mode
            was requested but degraded to template mode due to
            InsufficientNarrativeError (spec).
        briefing_id: Database row id of the persisted daily_briefings record.
            ...
            Protocol level in that case). Allows callers (API) to
    ...
    Naming conflict (Rule 7 — exposed):
        ...
        DTO uses `date` per spec. The conflict is documented
        here; legacy Briefing is a DB entity, BriefingResult is a DTO —
        they serve different layers and should not be unified in.
    """
