...
"""Trends API endpoints (T013 / R-sentiment-003).

Endpoints:
- GET /api/v1/trends/sentiment — sentiment trend analysis for an entity

Service construction (lazy pattern, mirrors briefings.py):
    SentimentTrendAnalyzer is not registered in the container; instead
    ``_get_sentiment_trend_service`` lazy-constructs it from the container's
    relational_pool. Tests patch ``api.endpoints.trends._get_sentiment_trend_service``.
...
Spec conflict (Rule 7 — exposed):
    ...
    endpoint only exposes ``entity`` (no community_id param), so entity is
    declared Optional in the signature (spec compliance) but the handler
    returns HTTP 400 when entity is missing or empty (Constraints compliance
    + user task spec: "entity 必传 (HTTP 400 if missing)"). This is the
    deliberate resolution — cover the scenario explicitly (Rule 24) rather
    than silently forwarding None to the service.
"""
