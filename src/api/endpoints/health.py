...
async def check_redis_health(client: Any, service_name: str = "redis") -> dict[str, Any]:
...
async def health_check() -> HealthCheckResponse:
    ...
    async def _check_relational() -> None:
    ...
    async def _check_cache() -> None:
        ...
        cache_service_name = (
            cache_client.cache_type if hasattr(cache_client, "cache_type") else "redis"
        )
        if cache_client is not None:
            redis_result = await check_redis_health(cache_client, service_name=cache_service_name)
    ...
    # Independent checks run concurrently so worst-case latency is one
    # check timeout (5s) instead of the sum of all three (15s).
    await asyncio.gather(_check_relational(), _check_graph(), _check_cache())
