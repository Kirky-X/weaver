...
def upgrade() -> None:
    ...
    persist_status = postgresql.ENUM(
        ...
        name='persist_status', create_type=False
    )
    emotion_type = postgresql.ENUM(
        '乐观', '振奋', '期待', '平静', '客观', '担忧', '悲观', '愤怒', '恐慌',
        name='emotion_type', create_type=False
    )
    vector_type = postgresql.ENUM(
        'title', 'content',
        name='vector_type', create_type=False
    )
    ...
    op.create_table(
        ...
        sa.Column('persist_status', persist_status, nullable=False,
        ...
        sa.CheckConstraint('sentiment_score >= 0 AND sentiment_score <= 1',
                           name='chk_sentiment_score_range'),
        sa.CheckConstraint('credibility_score >= 0 AND credibility_score <= 1',
                           name='chk_credibility_score_range'),
        sa.CheckConstraint('merged_into IS DISTINCT FROM id',
                           name='chk_no_self_merge'),
    )
    ...
    op.create_index('idx_av_unique', 'article_vectors',
                    ['article_id', 'vector_type'], unique=True)
