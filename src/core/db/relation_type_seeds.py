...
"""Canonical relation-type seed data.

Single source of truth for the default relation-type vocabulary. Imported by:
- ``core.db.duckdb_schema`` — seeds the DuckDB ``relation_types`` table.
- ``modules.processing.nodes.extraction.entity_extractor`` — builds the prompt
  fallback block when the DB-backed ``RelationTypeNormalizer`` is unavailable,
  so the fallback can never drift from what the table is seeded with.

NOTE: the Alembic migration ``23_seed_relation_types`` intentionally keeps its
own point-in-time copy so historical migrations stay reproducible.

This module is pure data with no imports on purpose: it must be importable
from inside the ``core.db`` package init without creating an import cycle.
"""

from __future__ import annotations

RELATION_TYPE_SEEDS: list[dict] = [
    # --- 组织 ---
    {
        "name": "任职于",
        "name_en": "WORKS_AT",
