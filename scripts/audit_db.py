...
NOT_NULL_FIELDS: dict[str, list[str]] = {
    "articles_core": [
        "id", "source_url", "title", "persist_status",
        "is_merged", "version", "document_type", "doc_metadata",
    ],
    "article_bodies": ["article_id", "body"],
    "article_analysis": [
        "article_id", "is_news", "verified_by_sources",
        "data_conflicts", "image_forensics",
    ],
    ...
    "source_configs": [
        ...
        "interval_minutes", "per_host_concurrency",
    ],
    "source_authorities": ["host"],
    "llm_usage_raw": [
        "id", "label", "call_point", "llm_type", "provider", "model",
        "input_tokens", "output_tokens", "total_tokens",
        "cached_tokens", "reasoning_tokens", "cost_usd", "latency_ms", "success",
    ],
    "llm_usage_hourly": [
        "id", "time_bucket", "label", "call_point", "llm_type", "provider", "model",
        "call_count", "input_tokens_sum", "output_tokens_sum", "total_tokens_sum",
        "cached_tokens_sum", "reasoning_tokens_sum", "cost_usd_sum",
        "latency_avg_ms", "latency_min_ms", "latency_max_ms",
        "success_count", "failure_count",
    ],
    "llm_failure_records": ["id", "call_point", "provider", "error_type"],
    "llm_compare_hourly": [
        "id", "time_bucket", "call_point", "primary_model", "candidate_model",
        "comparison_count",
    ],
    "alert_rules": ["id", "entity_name", "metric", "operator", "threshold"],
    "alert_events": [
        "id", "rule_id", "entity_name", "metric_value", "triggered_at",
    ],
    "pending_sync": ["id", "article_id", "sync_type", "payload"],
    "saga_logs": [
        "id", "saga_id", "article_id", "step_name", "step_status", "started_at",
    ],
    "relation_types": ["id", "name", "name_en", "category"],
    "relation_type_aliases": ["id", "alias", "relation_type_id"],
    "unknown_relation_types": ["id", "raw_type"],
    "sentiment_shifts": [
        "id", "community_id", "shift_type", "direction", "magnitude",
        "confidence", "detected_at", "window_start", "window_end",
    ],
...
BUSINESS_FIELDS: dict[str, list[str]] = {
    "articles_core": [
        "source_host", "source_id", "category", "language", "region",
        "score", "sentiment_score", "credibility_score",
        "publish_time", "content_hash", "created_at", "updated_at",
    ],
    "article_bodies": ["summary"],
    "article_analysis": [
        "subjects", "key_data", "impact", "has_data", "quality_score",
        "sentiment", "primary_emotion", "source_credibility",
        "cross_verification", "content_check_score", "event_time",
    ],
...
ENUM_VALUES: dict[str, tuple[str, str, set[str]]] = {
    ...
    "persist_status": ("articles_core", "persist_status", {
        "pending", "processing", "pg_done", "neo4j_done", "ladybug_done",
        "neo4j_failed", "failed",
        "saga_started", "saga_pg_done", "saga_graph_done",
        "saga_indexed", "saga_completed", "saga_failed",
    }),
    "category": ("articles_core", "category", {
        "政治", "军事", "经济", "科技", "社会", "文化", "体育", "国际",
    }),
    "document_type": ("articles_core", "document_type", {
        "news", "blog", "report", "press_release", "social_media",
...
def phase_1a_not_null_checks(con: duckdb.DuckDBPyConnection) -> list[dict]:
    ...
    for table, fields in NOT_NULL_FIELDS.items():
        # Check table exists
        exists = safe_execute(con, f"""
            SELECT COUNT(*) FROM information_schema.tables
            WHERE table_schema='main' AND table_name='{table}'
        """)
        if not exists or exists[0][0] == 0:
            findings.append({
                "table": table, "check": "table_exists", "status": "MISSING",
                "severity": "HIGH", "detail": f"Table {table} does not exist",
            })
            continue
        ...
        if not count_result or count_result[0][0] == "__ERROR__":
            findings.append({
                "table": table, "check": "row_count", "status": "ERROR",
                "severity": "HIGH", "detail": count_result[0][1] if count_result else "unknown",
            })
            continue
        ...
        for field in fields:
            ...
            if null_count > 0:
                findings.append({
                    "table": table, "field": field, "check": "not_null_violation",
                    "status": "FAIL", "severity": "CRITICAL",
                    "null_count": null_count, "total_rows": total_rows,
...
def phase_1b_business_field_checks(con: duckdb.DuckDBPyConnection) -> list[dict]:
    ...
    for table, fields in BUSINESS_FIELDS.items():
        ...
        if total_rows == 0:
            findings.append({
                "table": table, "check": "empty_table", "status": "WARN",
                "severity": "MEDIUM", "detail": f"Table {table} has 0 rows",
            })
            continue
        ...
        for field in fields:
            ...
            if null_count > 0:
                findings.append({
                    "table": table, "field": field, "check": "business_null",
                    "status": "WARN", "severity": severity,
                    "null_count": null_count, "total_rows": total_rows,
                    "null_rate": null_rate,
                })
    return findings
...
def phase_1c_foreign_key_checks(con: duckdb.DuckDBPyConnection) -> list[dict]:
    ...
    for child_table, child_col, parent_table, parent_col in FOREIGN_KEYS:
        ...
        for t in (child_table, parent_table):
            ...
            if orphans > 0:
                findings.append({
                    "check": "fk_integrity", "status": "FAIL",
                    "severity": "HIGH",
                    "child_table": child_table, "child_col": child_col,
                    "parent_table": parent_table, "parent_col": parent_col,
...
def phase_1d_enum_and_rule_checks(con: duckdb.DuckDBPyConnection) -> list[dict]:
    ...
    # Enum value checks
    for enum_name, (table, column, allowed) in ENUM_VALUES.items():
        exists = safe_execute(con, f"""
            SELECT COUNT(*) FROM information_schema.columns
        ...
        if invalid:
            for val, cnt in invalid:
                findings.append({
                    "check": "enum_value", "status": "FAIL",
                    "severity": "MEDIUM",
                    "table": table, "column": column,
                    "invalid_value": val, "count": cnt,
    ...
    for table, col in [
        ...
        out_of_range = safe_execute(con, f"""
            SELECT COUNT(*) FROM {table}
            WHERE {col} IS NOT NULL AND ({col} < 0 OR {col} > 1)
        """)[0][0]
        if out_of_range > 0:
            findings.append({
                "check": "range_violation", "status": "FAIL",
                "severity": "MEDIUM",
    ...
    self_merge = safe_execute(con, """
        SELECT COUNT(*) FROM articles_core
        WHERE merged_into IS NOT NULL AND merged_into = id
    """)[0][0]
    if self_merge > 0:
        findings.append({
            "check": "self_merge", "status": "FAIL", "severity": "HIGH",
            "table": "articles_core", "violation_count": self_merge,
            "rule": "merged_into != id",
        })

    # Business rule: interval_minutes range 5-1440
    bad_interval = safe_execute(con, """
        SELECT COUNT(*) FROM source_configs
        WHERE interval_minutes < 5 OR interval_minutes > 1440
    """)[0][0]
    if bad_interval > 0:
        findings.append({
            "check": "interval_range", "status": "FAIL", "severity": "MEDIUM",
            "table": "source_configs", "violation_count": bad_interval,
        })

    # Business rule: source_configs.tier range 1-3
    bad_tier = safe_execute(con, """
        SELECT COUNT(*) FROM source_configs
        WHERE tier IS NOT NULL AND (tier < 1 OR tier > 3)
    """)[0][0]
    if bad_tier > 0:
        findings.append({
            "check": "tier_range", "status": "FAIL", "severity": "MEDIUM",
            "table": "source_configs", "violation_count": bad_tier,
        })
...
def phase_1e_sampling_check(con: duckdb.DuckDBPyConnection) -> list[dict]:
    ...
    fields = ["source_url", "source_id", "title", "category", "language",
              "region", "score", "persist_status", "publish_time",
    ...
    for f in fields:
        if null_counts[f] > 0:
            ...
            findings.append({
                "check": "sample_null", "status": "WARN",
                "severity": severity,
                "field": f, "sample_size": sample_size,
                "null_in_sample": null_counts[f], "null_rate": rate,
            })

    # Check completed articles have all key fields
    completed_nulls = safe_execute(con, """
        SELECT
            COUNT(*) as total,
    ...
    if completed_nulls:
        ...
        if total_completed > 0:
            for label, cnt in [
                ("category", row[1]), ("language", row[2]),
                ("region", row[3]), ("score", row[4]),
                ("credibility_score", row[5]),
            ]:
                if cnt > 0:
                    findings.append({
                        "check": "completed_article_null", "status": "FAIL",
                        "severity": "HIGH",
                        "field": label,
